...
function CompetencyPicker({ employee, selected, onChange, tone, exclude = [], limit = 3 }: { employee: Employee; selected: string[]; onChange: (items: string[]) => void; tone: string; exclude?: string[]; limit?: number }) {
  ...
  return <div className="competency-list">{Object.entries(blockNames).map(([block, title]) => {
    ...
    return <div key={block}><h4>{title}</h4><div className="competency-compact-list">{items.map((item) => <div className={`competency-option ${selected.includes(item.id) ? "is-selected" : ""}`} key={item.id}><button type="button" className="competency-option__main" onClick={() => onChange(toggle(selected, item.id, limit))}><strong>{item.title}</strong><span>{title}</span></button><button type="button" className="info-button" aria-label={`Описание компетенции ${item.title}`} onClick={() => setInfo(item)}>i</button></div>)}</div></div>;
...
function SelfReviewEditor({ employee, review, onUpdate, notify }: { employee: Employee; review: SelfReview; onUpdate: (review: SelfReview) => void; notify: (message: string) => void }) {
  ...
  return <>
    ...
    {step === 2 && <Card title="Корпоративные навыки" eyebrow="Шаг 3 из 3" description="Выберите навыки из корпоративной модели, которые помогали вам в работе, и навыки, которые вы хотели бы усилить. Если нужно, нажмите на значок информации рядом с названием, чтобы посмотреть описание.">
      <div className="subsection"><h3>Выберите навыки, которые помогали вам добиться результата <small>от 1 до 3</small></h3><CompetencyPicker employee={employee} selected={review.corporateStrengths} onChange={(items) => set("corporateStrengths", items)} tone="strength" exclude={review.corporateDevelopment} limit={3} /></div>
      <div className="subsection"><h3>Выберите навыки, которые хотели бы усилить <small>от 1 до 3</small></h3><CompetencyPicker employee={employee} selected={review.corporateDevelopment} onChange={(items) => set("corporateDevelopment", items)} tone="development" exclude={review.corporateStrengths} limit={3} /></div>
    </Card>}
  ...
  return <div className="manager-page">
    ...
    <div className="split">
      <aside className="split__left">
        ...
        <SummaryTags title="Корпоративные навыки · помогали результату" items={selfReview.corporateStrengths.map(itemTitle)} tone="strength" />
        <SummaryTags title="Корпоративные навыки · хочет усилить" items={selfReview.corporateDevelopment.map(itemTitle)} tone="development" />
      </aside>
      <section className="split__right">
        ...
        <Card title="Корпоративные навыки" description="Сравните выбор сотрудника со своими наблюдениями. Вы можете подтвердить выбранные сотрудником навыки или выбрать другие.">
          <h3>Выберите навыки, которые помогали сотруднику добиться результата</h3><QuickCopy label="Подтвердить выбор сотрудника" onClick={() => setWithDraft({ corporateStrengths: [...new Set([...review.corporateStrengths, ...selfReview.corporateStrengths])].slice(0, 3) })} /><CompetencyPicker employee={employee} selected={review.corporateStrengths} onChange={(items) => setWithDraft({ corporateStrengths: items })} tone="manager-strength" exclude={review.corporateDevelopment} limit={3} />
          <div className="divider" /><h3>Выберите навыки, которые сотруднику стоит усилить</h3><QuickCopy label="Подтвердить выбор сотрудника" onClick={() => setWithDraft({ corporateDevelopment: [...new Set([...review.corporateDevelopment, ...selfReview.corporateDevelopment])].slice(0, 3) })} /><CompetencyPicker employee={employee} selected={review.corporateDevelopment} onChange={(items) => setWithDraft({ corporateDevelopment: items })} tone="manager-development" exclude={review.corporateStrengths} limit={3} />
        </Card>
